...
public class Data {
	...
	public void dataPrepareFromArff(String filePath) throws IOException{
		...
		ArffLoader loader = new ArffLoader(); // weka 读取数据
		loader.setFile(new File(filePath));
		
		Instances instances = loader.getDataSet(); //储存数据 instances 需要实例化
		this.instances = instances;   //将局部变量的值传给成员变量
		instances.setClassIndex(instances.numAttributes()-1); //默认最后一个属性是分类属性
		nFeatures = instances.numAttributes() - 1;  //列数
		...
		for(int i = 1; i < nFeatures; i++) {
			firstValueIndex[i] = firstValueIndex[i-1] + instances.attribute(i-1).numValues();  ///第i列的value数
		}
		...
		valueFrequency = new int[nValues];      //值的频率
		coOccurrence = new int[nValues][nValues];  // /同时出现频率
		
		
		
		
		for(int i = 0; i < nObjects; i++) {
			Instance instance = instances.instance(i);   //第i行数据
